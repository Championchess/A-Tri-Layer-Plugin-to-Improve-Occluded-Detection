...
class ConnectedOccluderMaskTestMixin(object):
    ...
    def simple_test_mask(self,
        ...
        mask_rois = torch.cat([batch_index, det_bboxes], dim=-1)
        mask_rois = mask_rois.view(-1, 5)
        front_mask_results = self._occluder_mask_forward(x, mask_rois)
        # back_mask_results = self._back_mask_forward(x, mask_rois)
        mask_results = self._mask_forward(x, mask_rois, middle_feats=front_mask_results['middle_feat'])
        mask_pred = mask_results['mask_pred']
        ...
        mask_preds = mask_pred.reshape(batch_size, num_proposals_per_img,
                                       *mask_pred.shape[1:])
        if self.return_test_occ_mask == True:
            front_mask_preds = front_mask_pred.reshape(batch_size, num_proposals_per_img,
                                       *front_mask_pred.shape[1:])
        ...
        for i in range(batch_size):
            mask_pred = mask_preds[i]
            if self.return_test_occ_mask == True:
                front_mask_pred = front_mask_preds[i]
            det_bbox = det_bboxes[i]
            det_label = det_labels[i]
            ...
            mask_pred = mask_pred[supplement_mask]
            if self.return_test_occ_mask == True:
                front_mask_pred = front_mask_pred[supplement_mask]
            det_bbox = det_bbox[supplement_mask]
            det_label = det_label[supplement_mask]
            ...
            if det_label.shape[0] == 0:
                ...
                segm_results.append(segm_result)
                if self.return_test_occ_mask == True:
                    front_segm_result = self.occluder_mask_head.get_seg_masks(
                        front_mask_pred, det_bbox, det_label, self.test_cfg,
                        ori_shapes[i], scale_factors[i], rescale)
                    front_segm_results.append(front_segm_result)
                # ipdb.set_trace()       
        
        if self.return_test_occ_mask == True:
            return segm_results, front_segm_results
        else:
            return segm_results
...
class TriConnectedOccluderMaskTestMixin(object):
    ...
    def simple_test_mask(self,
        ...
        back_mask_results = self._back_mask_forward(x, mask_rois)
        # ipdb.set_trace()
        mask_results = self._mask_forward(x, mask_rois, middle_feats=front_mask_results['middle_feat'] + back_mask_results['middle_feat'])
        mask_pred = mask_results['mask_pred']
        if self.return_test_occ_mask == True:
            front_mask_pred = front_mask_results['mask_pred']
        ...
        if self.return_test_occ_mask == True:
            front_mask_preds = front_mask_pred.reshape(batch_size, num_proposals_per_img,
                                       *front_mask_pred.shape[1:])
            back_mask_preds = back_mask_pred.reshape(batch_size, num_proposals_per_img,
                                       *back_mask_pred.shape[1:])
        ...
        for i in range(batch_size):
            ...
            if self.return_test_occ_mask == True:
                front_mask_pred = front_mask_preds[i]
                back_mask_pred = back_mask_preds[i]
            det_bbox = det_bboxes[i]
            det_label = det_labels[i]
            ...
            if self.return_test_occ_mask == True:
                front_mask_pred = front_mask_pred[supplement_mask]
                back_mask_pred = back_mask_pred[supplement_mask]
            det_bbox = det_bbox[supplement_mask]
            det_label = det_label[supplement_mask]
            ...
            if det_label.shape[0] == 0:
                ...
                segm_results.append(segm_result)
                if self.return_test_occ_mask == True:
                    front_segm_result = self.front_mask_head.get_seg_masks(
                        front_mask_pred, det_bbox, det_label, self.test_cfg,
                        ori_shapes[i], scale_factors[i], rescale)
                    back_segm_result = self.back_mask_head.get_seg_masks(
                        back_mask_pred, det_bbox, det_label, self.test_cfg,
                        ori_shapes[i], scale_factors[i], rescale)
                    front_segm_results.append(front_segm_result)
        ...
        if self.return_test_occ_mask == True:
            return segm_results, front_segm_results, back_segm_results
        else:
            return segm_results
...
class TriCatConnectedOccluderMaskTestMixin(object):
    ...
    def simple_test_mask(self,
        ...
        front_mask_results = self._front_mask_forward(x, mask_rois)
        back_mask_results = self._back_mask_forward(x, mask_rois)
        front_mask_dsample = F.interpolate(torch.sigmoid(front_mask_results['mask_pred']), 14, mode='bilinear')
        back_mask_dsample = F.interpolate(torch.sigmoid(back_mask_results['mask_pred']), 14, mode='bilinear')
        mask_results = self._mask_forward(x, mask_rois, middle_feats=torch.cat([front_mask_dsample, back_mask_dsample], dim=1))
        mask_pred = mask_results['mask_pred']
        if self.return_test_occ_mask == True:
            front_mask_pred = front_mask_results['mask_pred']
            back_mask_pred = back_mask_results['mask_pred']
...
class TriCat768ConnectedOccluderMaskTestMixin(object):
