...
class TriOccluderCascadeRoIHead(TriOccluderBaseRoIHead, BBoxTestMixin, TriCat768ConnectedOccluderMaskTestMixin):
    ...
    def __init__(self,
        ...
        # self.
        super(TriOccluderCascadeRoIHead, self).__init__(
    ...
    def forward_dummy(self, x, proposals):
        ...
        if self.with_mask:
            ...
            for i in range(self.num_stages):
                ...
                mask_results = self._mask_forward(i, x, mask_rois, middle_feat=torch.cat([front_mask_results['middle_feat'], back_mask_results['middle_feat']], dim=1))
    ...
    def forward_train(self,
        ...
        for i in range(self.num_stages):
            ...
            if self.forward_bbox_2_mask:
                # forward the box predictions to mask heads
                # refine bboxes
            ...
            if not self.forward_bbox_2_mask:
                # otherwise, update the boxes at the end of the iteration
                # refine bboxes
    ...
    def simple_test(self, x, proposal_list, img_metas, rescale=False):
        ...
        if self.with_mask:
            if all(det_bbox.shape[0] == 0 for det_bbox in det_bboxes):
                ...
                front_segm_results = [[[] for _ in range(mask_classes)]
                                for _ in range(num_imgs)]
                back_segm_results = [[[] for _ in range(mask_classes)]
                                for _ in range(num_imgs)]
            else:
                    ...
                    if det_bboxes[i].shape[0] == 0:
                        ...
                        if self.cascade_return_test_occ_mask == True:
                            ...
                            front_merged_masks = merge_aug_masks(
                                ...
                                rcnn_test_cfg)
                            front_segm_result = self.front_mask_head[-1].get_seg_masks(
                            ...
                            back_merged_masks = merge_aug_masks(
                                ...
                                rcnn_test_cfg)
                            back_segm_result = self.back_mask_head[-1].get_seg_masks(
